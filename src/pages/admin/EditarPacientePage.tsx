// src/pages/admin/EditarPacientePage.tsx
...
export function EditarPacientePage() {
  ...
  const { register, handleSubmit, formState: { errors }, reset } = useForm<PacienteForm>({
    resolver: zodResolver(pacienteSchema),
  })

  useEffect(() => {
    if (id) buscarPaciente()
  }, [id])
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={() => navigate(`/admin/pacientes/${id}`)}>
          <ArrowLeft className="w-5 h-5" />
        </Button>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Editar Paciente</h1>
          <p className="text-gray-600">Atualizar dados do paciente</p>
        </div>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              ...
              <Input label="CEP" placeholder="01234-567" {...register('cep')} />
            </div>
          </CardContent>
        </Card>

        <div className="flex justify-end gap-4">
          <Button type="button" variant="outline" onClick={() => navigate(`/admin/pacientes/${id}`)}>
            Cancelar
          </Button>
