// src/pages/admin/NovoMedicoPage.tsx
...
const medicoSchema = z.object({
  ...
  valor_consulta: z.string().min(1, 'Valor inválido'),
})

type MedicoForm = z.infer<typeof medicoSchema>

export function NovoMedicoPage() {
  const navigate = useNavigate()
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')

  const { register, handleSubmit, formState: { errors } } = useForm<MedicoForm>({
  ...
  const onSubmit = async (data: MedicoForm) => {
    ...
    try {
      ...
      setError(err.message || 'Erro ao cadastrar médico')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-600">Cadastrar novo médico</p>
        </div>
      </div>
